...
export interface JourneyRecovery { journeyId: string; reason: string; failedCapabilityId: string; replacementCapabilityId: string; state: "recovery" | "reassigned" | "resumed"; }
...
export class CharterJourneyRecoveryRuntime {
  ...
  async begin(input: JourneyExecution, failedCapabilityId: string, reason: string): Promise<JourneyRecovery> {
    ...
    return { journeyId: input.journeyId, reason, failedCapabilityId, replacementCapabilityId: "", state: "recovery" };
  }
