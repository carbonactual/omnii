...
export class TaskQueueRuntime {
  ...
  async heartbeat(taskId: string, workerId: string, leaseSeconds = 300): Promise<ProcessTask> { const task = await this.require(taskId); this.assertOwner(task, workerId); if (task.status !== "in_progress") throw new Error("Only in-progress tasks can heartbeat"); const updated = await this.persistence.updateIfVersion(TASK_COLLECTION, task.id, String(task.version ?? "1"), { lease_expires_at: new Date(Date.now() + Math.max(1, leaseSeconds) * 1000).toISOString() }); await this.events.append({ type: "PROCESS_TASK_HEARTBEAT", actor: workerId, subject: task.id, outcome: "renewed", provenance: { process_id: task.process_id, stage: task.stage }, payload: {}, idempotency_key: `process-task:heartbeat:${task.id}:${updated.version}` }); return structuredClone(updated as unknown as ProcessTask); }
  async approve(taskId: string, approverId: string, evidence: JsonObject[] = []): Promise<ProcessTask> { const task = await this.require(taskId); if (task.status !== "blocked") throw new Error("Only blocked tasks can be approved"); const updated = await this.persistence.updateIfVersion(TASK_COLLECTION, task.id, String(task.version ?? "1"), { status: "ready", outcome: { ...task.outcome, approval: { approved: true, approverId, at: new Date().toISOString() } }, evidence: [...task.evidence, ...evidence] }); await this.events.append({ type: "PROCESS_TASK_APPROVED", actor: approverId, subject: task.id, outcome: "ready", provenance: { process_id: task.process_id, stage: task.stage }, payload: { evidence_count: evidence.length }, idempotency_key: `process-task:approve:${task.id}:${approverId}` }); return structuredClone(updated as unknown as ProcessTask); }
  async reject(taskId: string, actorId: string, reason: string, evidence: JsonObject[] = []): Promise<ProcessTask> { const task = await this.require(taskId); if (["completed","rejected","skipped"].includes(task.status)) throw new Error("Terminal task cannot be rejected"); const updated = await this.persistence.updateIfVersion(TASK_COLLECTION, task.id, String(task.version ?? "1"), { status: "rejected", outcome: { ...task.outcome, rejection: { reason, actorId, at: new Date().toISOString() } }, evidence: [...task.evidence, ...evidence], completed_at: new Date().toISOString() }); await this.events.append({ type: "PROCESS_TASK_REJECTED", actor: actorId, subject: task.id, outcome: "rejected", provenance: { process_id: task.process_id, stage: task.stage }, payload: { reason }, idempotency_key: `process-task:reject:${task.id}` }); return structuredClone(updated as unknown as ProcessTask); }
  async complete(input: TaskCompletionInput): Promise<ProcessTask> { const task = await this.require(input.taskId); this.assertOwner(task, input.workerId); if (task.status !== "in_progress") throw new Error("Only in-progress tasks can complete"); const evidence = input.evidence ?? []; this.assertEvidence(this.requiredEvidence(task), evidence, task); const status = input.status ?? "completed"; const updated = await this.persistence.updateIfVersion(TASK_COLLECTION, task.id, String(task.version ?? "1"), { status, outcome: input.outcome ?? {}, evidence: [...task.evidence, ...evidence], completed_at: new Date().toISOString(), lease_expires_at: null, claimed_by: null }); await this.events.append({ type: "PROCESS_TASK_COMPLETED", actor: input.workerId, subject: task.id, outcome: status, provenance: { process_id: task.process_id, stage: task.stage }, payload: { evidence_count: evidence.length }, idempotency_key: `process-task:complete:${task.id}` }); return structuredClone(updated as unknown as ProcessTask); }
  async fail(input: TaskFailureInput, policy: TaskQueuePolicy): Promise<ProcessTask> { const task = await this.require(input.taskId); this.assertOwner(task, input.workerId); if (task.status !== "in_progress") throw new Error("Only in-progress tasks can fail"); const attempt = Number(task.attempt_count ?? 1); const exhausted = attempt >= Math.max(1, policy.maxAttempts); const nextStatus: ProcessTaskStatus = exhausted ? "blocked" : "ready"; const nextAttemptAt = exhausted ? null : new Date(Date.now() + this.backoffMs(attempt)).toISOString(); const updated = await this.persistence.updateIfVersion(TASK_COLLECTION, task.id, String(task.version ?? "1"), { status: nextStatus, attempt_count: attempt, max_attempts: policy.maxAttempts, last_error: input.error, next_attempt_at: nextAttemptAt, lease_expires_at: null, claimed_by: null }); await this.events.append({ type: exhausted ? "PROCESS_TASK_RETRY_EXHAUSTED" : "PROCESS_TASK_RETRY_SCHEDULED", actor: input.workerId, subject: task.id, outcome: nextStatus, provenance: { process_id: task.process_id, stage: task.stage }, payload: { attempt, next_attempt_at: nextAttemptAt, error: input.error }, idempotency_key: `process-task:failure:${task.id}:${attempt}` }); return structuredClone(updated as unknown as ProcessTask); }
  async escalate(taskId: string, actorId: string, reason: string, targetAssigneeId?: string): Promise<ProcessTask> { const task = await this.require(taskId); if (["completed","rejected","skipped"].includes(task.status)) throw new Error("Terminal task cannot escalate"); const level = Number(task.escalation_level ?? 0) + 1; const updated = await this.persistence.updateIfVersion(TASK_COLLECTION, task.id, String(task.version ?? "1"), { status: "blocked", assignee_id: targetAssigneeId ?? task.assignee_id ?? null, escalation_level: level, escalation_reason: reason, escalation_at: new Date().toISOString(), claimed_by: null, lease_expires_at: null }); await this.events.append({ type: "PROCESS_TASK_ESCALATED", actor: actorId, subject: task.id, outcome: "blocked", provenance: { process_id: task.process_id, stage: task.stage }, payload: { level, targetAssigneeId: targetAssigneeId ?? null, reason }, idempotency_key: `process-task:escalate:${task.id}:${level}` }); return structuredClone(updated as unknown as ProcessTask); }
