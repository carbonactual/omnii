...
create unique index if not exists omnii_events_idempotency_key_idx
  on omnii_events ((payload->>'idempotency_key'))
  where payload ? 'idempotency_key';

create unique index if not exists omnii_executions_idempotency_key_idx
  on omnii_executions ((payload->>'idempotencyKey'))
  where payload ? 'idempotencyKey';

create unique index if not exists omnii_ledger_idempotency_key_idx
  on omnii_ledger ((payload->>'idempotencyKey'))
  where payload ? 'idempotencyKey';
...
begin
  ...
  insert into omnii_events (id, version, lifecycle, authority, provenance, payload, correlation_id)
  values (
    p_event_id, '1', 'active', p_event_provenance->'authority', p_event_provenance,
  ...
  returning jsonb_build_object('id', id, 'correlation_id', correlation_id, 'payload', payload) into v_event;

  return v_event;
end;
$$;

create or replace function omnii_atomic_execution_audit(
  p_execution_id text,
...
begin
  ...
  if not found then
    raise exception 'OMNII execution record not found: %', p_execution_id;
  end if;

  insert into omnii_audit (id, version, lifecycle, authority, provenance, payload)
...
create or replace function omnii_atomic_ledger_audit(
  p_ledger_id text,
  p_ledger_authority jsonb,
  p_ledger_provenance jsonb,
...
as $$
begin
  insert into omnii_ledger (id, version, lifecycle, authority, provenance, payload)
  values (p_ledger_id, '1', 'active', p_ledger_authority, p_ledger_provenance, p_ledger_payload);
