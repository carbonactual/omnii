...
export type EventAppendInput<T extends JsonObject = JsonObject> = Omit<
  CanonicalEvent<T>, "id" | "recorded_at" | "event_hash" | "payload"
> & { id?: string; recorded_at?: string; payload: T };
...
function sortedJson(value: unknown): unknown {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value as Record<string, unknown>)
      .sort(([a], [b]) => a.localeCompare(b)).map(([key, child]) => [key, sortedJson(child)]));
  }
  return value;
}
...
function semanticMaterial(event: Omit<CanonicalEvent, "event_hash" | "id" | "recorded_at">): unknown {
  return {
    ...
    actor_ref: event.actor_ref, subject_ref: event.subject_ref, institution_ref: event.institution_ref,
    operating_context_id: event.operating_context_id, correlation_id: event.correlation_id,
    causation_id: event.causation_id, parent_event_id: event.parent_event_id,
    reality_state: event.reality_state, authority_ref: event.authority_ref, source: event.source,
...
export function normalizeEventInput<T extends JsonObject>(input: EventAppendInput<T>): CanonicalEvent<T> {
  assertNonEmpty("event_type", input.event_type); assertNonEmpty("source", input.source);
  assertNonEmpty("correlation_id", input.correlation_id); assertNonEmpty("idempotency_key", input.idempotency_key);
  assertNonEmpty("event_version", input.event_version); assertNonEmpty("schema_version", input.schema_version);
  ...
  const eventWithoutHash: Omit<CanonicalEvent<T>, "event_hash"> = {
    id: input.id ?? randomUUID(), event_type: input.event_type, event_version: input.event_version,
    schema_version: input.schema_version, lifecycle: input.lifecycle ?? "active", status: input.status ?? "accepted",
    occurred_at: occurredAt.toISOString(), recorded_at: recordedAt.toISOString(), actor_ref: input.actor_ref,
    subject_ref: input.subject_ref, institution_ref: input.institution_ref, operating_context_id: input.operating_context_id,
    correlation_id: input.correlation_id, causation_id: input.causation_id, parent_event_id: input.parent_event_id,
    reality_state: realityState, authority_ref: input.authority_ref, source: input.source,
    provenance: clone(input.provenance ?? {}), evidence_refs: clone(input.evidence_refs ?? []),
...
function fromPersistence(record: Record<string, unknown>): CanonicalEvent {
  ...
  const normalizedInput: EventAppendInput = {
    id: String(record.id), event_type: eventType, event_version: String(record.event_version ?? record.version ?? "1"),
    schema_version: String(record.schema_version ?? "1"), lifecycle: String(record.lifecycle ?? "active"),
    status: String(record.status ?? "accepted"), occurred_at: String(record.occurred_at ?? record.created_at),
    recorded_at: String(record.recorded_at ?? record.created_at),
    ...
    parent_event_id: typeof record.parent_event_id === "string" ? record.parent_event_id : undefined,
    reality_state: isEventRealityState(record.reality_state) ? record.reality_state : "unknown",
    authority_ref: typeof record.authority_ref === "string" ? record.authority_ref : undefined,
    source: String(record.source ?? (record.provenance as Record<string, unknown> | undefined)?.source ?? "legacy"),
    provenance: clone((record.provenance ?? {}) as JsonObject), evidence_refs: clone((record.evidence_refs ?? []) as unknown[]),
    metadata: clone((record.metadata ?? {}) as JsonObject), payload,
...
export class EventEngine {
  ...
  async append<T extends JsonObject>(input: EventAppendInput<T>): Promise<CanonicalEvent<T>> {
    ...
    const existing = await this.findByIdempotencyKey(normalized.idempotency_key);
    if (existing) {
      if (existing.event_hash !== normalized.event_hash) throw new Error("omnii_event_idempotency_conflict");
      return clone(existing as CanonicalEvent<T>);
    }
  ...
  async appendMany<T extends JsonObject>(inputs: EventAppendInput<T>[]): Promise<CanonicalEvent<T>[]> {
    return this.persistence.transaction(async () => {
      const results: CanonicalEvent<T>[] = [];
      for (const input of inputs) results.push(await this.append(input));
      return results;
    });
  }
  ...
  async appendSignal<T extends JsonObject>(signal: RuntimeSignal<T>): Promise<CanonicalEvent<T>> {
    return this.append({
      event_type: signal.eventType, event_version: "1", schema_version: "1", lifecycle: "active", status: "accepted",
      occurred_at: signal.receivedAt, actor_ref: signal.actorId, subject_ref: signal.subjectId,
      institution_ref: signal.institutionId, operating_context_id: signal.operatingContextId, correlation_id: signal.correlationId,
      reality_state: "observed", source: signal.source, provenance: clone(signal.provenance ?? {}), evidence_refs: [],
  ...
  async get(id: string): Promise<CanonicalEvent | undefined> {
    const record = await this.persistence.read("events", id);
    return record ? clone(fromPersistence(record as Record<string, unknown>)) : undefined;
  }

  async findByIdempotencyKey(idempotencyKey: string): Promise<CanonicalEvent | undefined> {
    assertNonEmpty("idempotency_key", idempotencyKey);
  ...
  async query(filter: EventQuery = {}, limit = Number.MAX_SAFE_INTEGER): Promise<CanonicalEvent[]> {
    const events = await this.persistence.query("events", (record) => {
      ...
      if (filter.actor_ref && e.actor_ref !== filter.actor_ref) return false;
      if (filter.subject_ref && e.subject_ref !== filter.subject_ref) return false;
      if (filter.institution_ref && e.institution_ref !== filter.institution_ref) return false;
      if (filter.correlation_id && e.correlation_id !== filter.correlation_id) return false;
      if (filter.causation_id && e.causation_id !== filter.causation_id) return false;
      if (filter.parent_event_id && e.parent_event_id !== filter.parent_event_id) return false;
      if (filter.reality_state && e.reality_state !== filter.reality_state) return false;
      if (filter.source && e.source !== filter.source) return false;
      if (filter.occurred_from && e.occurred_at < filter.occurred_from) return false;
      if (filter.occurred_to && e.occurred_at > filter.occurred_to) return false;
      return true;
    });
    return events.map((record) => fromPersistence(record as Record<string, unknown>))
      .sort((a, b) => a.occurred_at.localeCompare(b.occurred_at) || a.recorded_at.localeCompare(b.recorded_at) || a.id.localeCompare(b.id))
  ...
  async replay(handler: (event: CanonicalEvent) => Promise<void> | void, options: EventReplayOptions = {}): Promise<number> {
    const events = await this.query(options.query, options.limit ?? Number.MAX_SAFE_INTEGER);
    for (const event of events) await handler(clone(event));
