...
begin
  ...
  loop
    begin
      ...
      if not found then
        ...
        set status = 'blocked',
            ...
            error_message = 'No active exact task handler binding exists for this task.',
            updated_at = now()
        where id = v_task.id;
      ...
      if v_handler.execution_mode <> 'deterministic' then
        update public.omnii_process_tasks
        set status = 'blocked',
            error_code = 'EXECUTION_MODE_NOT_AUTONOMOUS',
            error_message = 'Continuous worker will not execute non-deterministic handlers.',
            updated_at = now()
        where id = v_task.id;
        v_blocked := v_blocked + 1;
        continue;
      ...
      if coalesce(jsonb_array_length(coalesce(v_handler.authority_requirements, '[]'::jsonb)), 0) > 0 then
        ...
        set status = 'blocked',
            ...
            error_message = 'Human or delegated authority is required before this task can execute.',
            updated_at = now()
        where id = v_task.id;
        v_blocked := v_blocked + 1;
        continue;
      ...
      -- Current autonomous handler is workflow_stage. Its result is deliberately
      -- bounded to a deterministic acknowledgement/evidence envelope; it does not
      -- acquire authority or invent business facts.
      if v_handler.handler_key <> 'workflow_stage' then
        update public.omnii_process_tasks
        set status = 'blocked',
            error_code = 'HANDLER_NOT_ALLOWLISTED',
            error_message = 'Handler is registered but not allowlisted for autonomous execution.',
            updated_at = now()
        where id = v_task.id;
        v_blocked := v_blocked + 1;
        continue;
      ...
      v_evidence := coalesce(v_handler.evidence_template, '{}'::jsonb) ||
        jsonb_build_object(
          'scheduler', 'omnii_worker_tick',
          'handler_key', v_handler.handler_key,
          'executed_at', now()
        );
      v_outcome := jsonb_build_object(
        'status', 'completed',
        'outcome_event', v_handler.outcome_event,
        'handler_key', v_handler.handler_key
      );
      ...
      v_progress := public.omnii_progress_process(
        v_task.process_instance_id,
        v_handler.outcome_event,
        v_outcome
      );
    ...
    exception when others then
      update public.omnii_process_tasks
      set status = 'blocked',
          error_code = 'WORKER_TICK_EXCEPTION',
...
-- Schedule every 30 seconds. pg_cron executes the function in-database, avoiding
-- an ungoverned network hop and preserving the existing queue/progression boundary.
do $$
begin
