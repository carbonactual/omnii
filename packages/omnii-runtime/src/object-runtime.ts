...
export class ObjectRuntime {
  ...
  async create(input: Omit<CanonicalObject, "id" | "version" | "timestamps"> & Partial<Pick<CanonicalObject, "id" | "version" | "timestamps">>): Promise<CanonicalObject> {
    ...
    const object: CanonicalObject = { ...input, id: input.id ?? randomUUID(), version: input.version ?? "1", timestamps: input.timestamps ?? { created_at: now, updated_at: now } };
  ...
  async update(id: string, patch: Partial<Omit<CanonicalObject, "id">>, expectedVersion?: string): Promise<CanonicalObject> {
    ...
    if (current.version !== expected) throw new Error(`Object version conflict: expected ${expected}, found ${current.version}`);
    const updated: CanonicalObject = { ...current, ...patch, id, version: String(Number(current.version) + 1), timestamps: { ...current.timestamps, updated_at: new Date().toISOString() } };
    assertValid(validateCanonicalObject(updated));
    const persisted = await this.persistence.updateIfVersion("objects", id, expected, updated);
