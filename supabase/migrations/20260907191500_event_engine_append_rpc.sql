  ...
  select encode(
    digest(
      jsonb_build_object(
        ...
        'lifecycle', p_lifecycle, 'status', p_status, 'occurred_at', p_occurred_at,
        'actor_ref', p_actor_ref, 'subject_ref', p_subject_ref, 'institution_ref', p_institution_ref,
        'operating_context_id', p_operating_context_id, 'correlation_id', p_correlation_id,
        'causation_id', p_causation_id, 'parent_event_id', p_parent_event_id,
        'reality_state', p_reality_state, 'authority_ref', p_authority_ref, 'source', p_source,
        'provenance', p_provenance, 'evidence_refs', p_evidence_refs, 'metadata', p_metadata,
...
set event_hash = public.omnii_event_semantic_hash(
  event_type, event_version, schema_version, lifecycle, status, occurred_at,
  actor_ref, subject_ref, institution_ref, operating_context_id, correlation_id,
  causation_id, parent_event_id, reality_state, authority_ref, source,
  provenance, evidence_refs, metadata, payload, idempotency_key
)
...
begin
  ...
  v_hash := public.omnii_event_semantic_hash(
    p_event_type, v_event_version, v_schema_version, v_lifecycle, v_status, v_occurred_at,
    p_actor_ref, p_subject_ref, p_institution_ref, p_operating_context_id,
    p_correlation_id, p_causation_id, p_parent_event_id, v_reality_state, p_authority_ref, p_source,
    v_provenance, v_evidence_refs, v_metadata, v_payload, p_idempotency_key
  );
  ...
  begin
    insert into public.omnii_events(
      ...
      coalesce(nullif(p_id,''), gen_random_uuid()::text), v_event_version, v_lifecycle,
      coalesce(v_provenance->'authority','{}'::jsonb), v_provenance, v_payload, p_correlation_id, p_idempotency_key,
      p_event_type, v_event_version, v_schema_version, v_occurred_at, v_recorded_at,
      p_actor_ref, p_subject_ref, p_institution_ref, p_operating_context_id,
      p_causation_id, p_parent_event_id, v_reality_state, p_authority_ref, p_source,
      v_evidence_refs, v_metadata, v_status, v_hash
