...
export class RuntimeActivation {
  ...
  constructor(private readonly deps: RuntimeActivationDependencies) {
    ...
    this.authorities = deps.authorityRuntime ?? new AuthorityRuntime({ persistence: this.persistence, events: this.events });
    this.executions = deps.executionRuntime ?? new ExecutionRuntime(this.events, this.persistence);
  }
  ...
  async activate(signal: RuntimeSignal): Promise<RuntimeActivationResult> {
    ...
    try { authority = await this.authorities.validate(authorityId, { context: { operatingContextId: context.id } }); } catch { return this.blocked(signal, "authority_invalid", eventIds); }
    ...
    const reconciliationEvent = await this.events.append({ type: "RUNTIME_RECONCILIATION", actor: route.actorIdentity, subject: completed.id, correlation_id: signal.correlationId, idempotency_key: `runtime:reconciliation:${signal.idempotencyKey}`, outcome: reconciliation.matched ? "matched" : "discrepancy", provenance: { signalId: signal.id, authorityId: authority.id }, payload: { expected, observed, discrepancy: reconciliation.discrepancy ?? null } });
    eventIds.push(reconciliationEvent.id);
    const feedback = this.feedback(completed, reconciliation);
    const feedbackEvent = await this.events.append({ type: "RUNTIME_FEEDBACK_OBSERVED", actor: route.actorIdentity, subject: completed.id, correlation_id: signal.correlationId, idempotency_key: `runtime:feedback:${signal.idempotencyKey}`, outcome: "observed", provenance: { signalId: signal.id, authorityId: authority.id }, payload: feedback as unknown as JsonObject });
  ...
  private async blocked(signal: RuntimeSignal, reason: string, eventIds: string[]): Promise<RuntimeActivationResult> { const blocked = await this.events.append({ type: "RUNTIME_EXECUTION_BLOCKED", actor: signal.actorId ?? signal.source, subject: signal.subjectId, correlation_id: signal.correlationId, idempotency_key: `runtime:blocked:${signal.idempotencyKey}`, outcome: "blocked", provenance: signal.provenance ?? {}, payload: { signalId: signal.id, reason } }); return { signal, resolution: { status: "blocked", reason }, eventIds: [...eventIds, blocked.id] }; }
