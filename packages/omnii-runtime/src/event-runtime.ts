...
export interface OmniiEvent { [key: string]: unknown; id: string; type: string; occurred_at: string; recorded_at?: string; actor: string; subject?: string; correlation_id?: string; idempotency_key?: string; outcome?: string; provenance: JsonObject; payload?: JsonObject; }

export class EventStore {
  constructor(private readonly persistence: PersistencePort = new MemoryPersistenceAdapter()) {}
  async append(input: Omit<OmniiEvent, "id" | "occurred_at" | "recorded_at">): Promise<OmniiEvent> { if (input.idempotency_key) { const existing = await this.persistence.query("events", (record) => record["idempotency_key"] === input.idempotency_key); if (existing.length) return structuredClone(existing[0] as unknown as OmniiEvent); } const event: OmniiEvent = { ...input, id: randomUUID(), occurred_at: new Date().toISOString(), recorded_at: new Date().toISOString() }; const created = await this.persistence.create("events", event); return structuredClone(created as unknown as OmniiEvent); }
