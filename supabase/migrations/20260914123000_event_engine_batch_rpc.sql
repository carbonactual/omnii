...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_events) loop
    ...
    from public.omnii_append_event(
      nullif(v_item->>'id',''), v_item->>'event_type', v_item->>'event_version', v_item->>'schema_version',
      v_item->>'lifecycle', v_item->>'status', (v_item->>'occurred_at')::timestamptz, (v_item->>'recorded_at')::timestamptz,
      v_item->>'actor_ref', v_item->>'subject_ref', v_item->>'institution_ref', v_item->>'operating_context_id',
      v_item->>'correlation_id', v_item->>'causation_id', v_item->>'parent_event_id', v_item->>'reality_state',
      v_item->>'authority_ref', v_item->>'source', coalesce(v_item->'provenance','{}'::jsonb),
      coalesce(v_item->'evidence_refs','[]'::jsonb), coalesce(v_item->'metadata','{}'::jsonb),
      coalesce(v_item->'payload','{}'::jsonb), v_item->>'idempotency_key'
    );
    return next v_event;
  end loop;
  return;
