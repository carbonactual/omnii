...
-- Add queryable envelope fields while preserving legacy payloads.
...
alter table public.omnii_events
  ...
  add column if not exists operating_context_id text,
...
alter table public.omnii_events drop constraint if exists omnii_events_reality_state_chk;
alter table public.omnii_events add constraint omnii_events_reality_state_chk
...
create index if not exists omnii_events_source_idx on public.omnii_events(source, occurred_at desc);
...
before update on public.omnii_events for each row execute function public.omnii_reject_event_mutation();

drop trigger if exists omnii_events_immutable_delete on public.omnii_events;
create trigger omnii_events_immutable_delete
before delete on public.omnii_events for each row execute function public.omnii_reject_event_mutation();
