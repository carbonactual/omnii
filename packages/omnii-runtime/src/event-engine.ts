...
export type EventAppendInput<T extends JsonObject = JsonObject> = Omit<
  CanonicalEvent<T>,
  "id" | "recorded_at" | "event_hash" | "payload"
> & {
  id?: string;
  recorded_at?: string;
  payload: T;
...
function calculateEventHash(event: Omit<CanonicalEvent, "event_hash">): string {
  const canonical = JSON.stringify(sortedJson(event));
...
export function normalizeEventInput<T extends JsonObject>(input: EventAppendInput<T>): CanonicalEvent<T> {
  assertNonEmpty("event_type", input.event_type);
  assertNonEmpty("source", input.source);
  assertNonEmpty("correlation_id", input.correlation_id);
  assertNonEmpty("idempotency_key", input.idempotency_key);
  assertNonEmpty("event_version", input.event_version);
  assertNonEmpty("schema_version", input.schema_version);
  ...
  if (Number.isNaN(occurredAt.getTime())) throw new Error("occurred_at must be a valid timestamp");
  const recordedAt = new Date(input.recorded_at ?? new Date().toISOString());
  if (Number.isNaN(recordedAt.getTime())) throw new Error("recorded_at must be a valid timestamp");

  const payload = clone(input.payload);
  if (typeof payload.type === "undefined") payload.type = input.event_type;
  if (typeof payload.type !== "string" || payload.type !== input.event_type) {
    throw new Error("payload.type must equal event_type");
  }
  ...
  const eventWithoutHash: Omit<CanonicalEvent<T>, "event_hash"> = {
    ...
    operating_context_id: input.operating_context_id,
...
function fromPersistence(record: Record<string, unknown>): CanonicalEvent {
  ...
  const normalizedInput: EventAppendInput = {
    ...
    event_version: String(record.event_version ?? record.version ?? "1"),
    schema_version: String(record.schema_version ?? "1"),
    lifecycle: String(record.lifecycle ?? "active"),
    status: String(record.status ?? "accepted"),
    occurred_at: String(record.occurred_at ?? record.created_at),
    ...
    correlation_id: String(record.correlation_id ?? record.id),
    ...
    provenance: clone((record.provenance ?? {}) as JsonObject),
    evidence_refs: clone((record.evidence_refs ?? []) as unknown[]),
    metadata: clone((record.metadata ?? {}) as JsonObject),
    payload,
...
export class EventEngine {
  ...
  async appendSignal<T extends JsonObject>(signal: RuntimeSignal<T>): Promise<CanonicalEvent<T>> {
    return this.append({
      ...
      actor_ref: signal.actorId,
      subject_ref: signal.subjectId,
      institution_ref: signal.institutionId,
      operating_context_id: signal.operatingContextId,
      correlation_id: signal.correlationId,
      reality_state: "observed",
      source: signal.source,
      provenance: clone(signal.provenance ?? {}),
      evidence_refs: [],
      metadata: { domain: signal.domain, location: clone(signal.location ?? {}) },
  ...
  async query(filter: EventQuery = {}, limit = Number.MAX_SAFE_INTEGER): Promise<CanonicalEvent[]> {
    ...
    return events
      .map((record) => fromPersistence(record as Record<string, unknown>))
