...
export interface CapabilityDescriptor {
  id: string; name: string; version: string; providerId: string; domain: string; operation: string;
  status: CapabilityStatus; authorityClass: CapabilityAuthorityClass; riskClass: CapabilityRiskClass;
  sideEffect: CapabilitySideEffect; identityScope: string; inputSchema: JsonObject; outputSchema: JsonObject;
  dependencies: string[]; costHint: number; latencyHint: number; reliabilityHint: number; auditPolicy: string;
  provenance: JsonObject; metadata?: JsonObject;
}
export interface CapabilityExecutionContext { principal: string; authorityRef?: string; correlationId: string; idempotencyKey: string; intent: JsonObject; provenance?: JsonObject; }
export interface CapabilityAdapterResult { success: boolean; output?: JsonObject; evidenceRefs?: string[]; providerReference?: string; error?: string; }
export interface CapabilityAdapter { descriptor: CapabilityDescriptor; execute(input: JsonObject, context: CapabilityExecutionContext): Promise<CapabilityAdapterResult>; }
...
export class CapabilityRegistryRuntime {
  ...
  async register(descriptor: CapabilityDescriptor, _actor: string): Promise<CapabilityDescriptor> {
    ...
    this.descriptors.set(key, structuredClone(descriptor)); return structuredClone(descriptor);
  }
  async resolve(id: string, providerId?: string): Promise<CapabilityDescriptor | undefined> {
    if (providerId) { const item = this.descriptors.get(`${id}::${providerId}`); return item ? structuredClone(item) : undefined; }
    const first = [...this.descriptors.values()].filter((item) => item.id === id).sort((a,b)=>a.providerId.localeCompare(b.providerId))[0];
  ...
  async lookup(predicate: (item: CapabilityDescriptor) => boolean): Promise<CapabilityDescriptor[]> {
    return [...this.descriptors.values()].filter(predicate).sort((a,b)=>a.id.localeCompare(b.id)||a.providerId.localeCompare(b.providerId)).map((item)=>structuredClone(item));
  }
  async active(id: string): Promise<CapabilityDescriptor[]> { return this.lookup((item)=>item.id===id && activeStatuses.has(item.status)); }
  async list(): Promise<CapabilityDescriptor[]> { return this.lookup(()=>true); }
  async deprecate(id: string, providerId: string): Promise<CapabilityDescriptor> {
    const key = `${id}::${providerId}`; const item = this.descriptors.get(key); if (!item) throw new Error("capability_not_found");
    const updated = {...item,status:"deprecated" as const}; this.descriptors.set(key,updated); return structuredClone(updated);
  }
}
...
export class CapabilityCatalogAdapter implements CapabilityCatalog {
  ...
  async lookup(predicate: (record: AbbaCapabilityRecord) => boolean): Promise<AbbaCapabilityRecord[]> {
    ...
    return descriptors.map((descriptor): AbbaCapabilityRecord => ({
      id: descriptor.id, name: descriptor.name, version: descriptor.version, status: descriptor.status,
      authority: { class: descriptor.authorityClass, granted: false },
      provenance: {...structuredClone(descriptor.provenance),provider_id:descriptor.providerId},
      constraints: {riskClass:descriptor.riskClass,sideEffect:descriptor.sideEffect,identityScope:descriptor.identityScope},
    })).filter(predicate).sort((a,b)=>a.id.localeCompare(b.id)||a.version.localeCompare(b.version));
  }
}
