...
function fromPersistence(record: Record<string, unknown>): CanonicalEvent {
  const payload = (record.payload ?? {}) as JsonObject; const payloadType = (payload as Record<string, unknown>).type;
  const eventType = String(record.event_type ?? (typeof payloadType === "string" ? payloadType : "UNKNOWN"));
  const payloadActor = (payload as Record<string, unknown>).actor; const payloadSubject = (payload as Record<string, unknown>).subject;
  const normalizedInput: EventAppendInput = {
    id: String(record.id), event_type: eventType, event_version: String(record.event_version ?? record.version ?? "1"), schema_version: String(record.schema_version ?? "1"), lifecycle: String(record.lifecycle ?? "active"), status: String(record.status ?? "accepted"), occurred_at: String(record.occurred_at ?? record.created_at), recorded_at: String(record.recorded_at ?? record.created_at), actor_ref: typeof record.actor_ref === "string" ? record.actor_ref : typeof payloadActor === "string" ? payloadActor : undefined, subject_ref: typeof record.subject_ref === "string" ? record.subject_ref : typeof payloadSubject === "string" ? payloadSubject : undefined, institution_ref: typeof record.institution_ref === "string" ? record.institution_ref : undefined, operating_context_id: typeof record.operating_context_id === "string" ? record.operating_context_id : undefined, correlation_id: String(record.correlation_id ?? record.id), causation_id: typeof record.causation_id === "string" ? record.causation_id : undefined, parent_event_id: typeof record.parent_event_id === "string" ? record.parent_event_id : undefined, reality_state: isEventRealityState(record.reality_state) ? record.reality_state : "unknown", authority_ref: typeof record.authority_ref === "string" ? record.authority_ref : undefined, source: String(record.source ?? (record.provenance as Record<string, unknown> | undefined)?.source ?? "legacy"), provenance: clone((record.provenance ?? {}) as JsonObject), evidence_refs: clone((record.evidence_refs ?? []) as unknown[]), metadata: clone((record.metadata ?? {}) as JsonObject), payload, idempotency_key: String(record.idempotency_key ?? `legacy:${record.id}`)
  };
...
export class EventEngine {
  ...
  async append<T extends JsonObject>(input: EventAppendInput<T>): Promise<CanonicalEvent<T>> {
    ...
    if (this.persistence.appendEvents) { const result = await this.persistence.appendEvents([normalized]); return clone(result[0] as CanonicalEvent<T>); }
    const created = await this.persistence.create("events", normalized as unknown as { id: string }); return clone(fromPersistence(created as Record<string, unknown>) as CanonicalEvent<T>);
  }
  async appendMany<T extends JsonObject>(inputs: EventAppendInput<T>[]): Promise<CanonicalEvent<T>[]> {
    const normalized = inputs.map(normalizeEventInput);
    if (this.persistence.appendEvents) { const result = await this.persistence.appendEvents(normalized); return clone(result as CanonicalEvent<T>[]); }
  ...
  async query(filter: EventQuery = {}, limit = Number.MAX_SAFE_INTEGER): Promise<CanonicalEvent[]> {
    const events = await this.persistence.query("events", (record) => { const e = fromPersistence(record as Record<string, unknown>); if (filter.event_type && e.event_type !== filter.event_type) return false; if (filter.actor_ref && e.actor_ref !== filter.actor_ref) return false; if (filter.subject_ref && e.subject_ref !== filter.subject_ref) return false; if (filter.institution_ref && e.institution_ref !== filter.institution_ref) return false; if (filter.correlation_id && e.correlation_id !== filter.correlation_id) return false; if (filter.causation_id && e.causation_id !== filter.causation_id) return false; if (filter.parent_event_id && e.parent_event_id !== filter.parent_event_id) return false; if (filter.reality_state && e.reality_state !== filter.reality_state) return false; if (filter.source && e.source !== filter.source) return false; if (filter.occurred_from && e.occurred_at < filter.occurred_from) return false; if (filter.occurred_to && e.occurred_at > filter.occurred_to) return false; return true; });
