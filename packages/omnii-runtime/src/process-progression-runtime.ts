...
export interface WorkflowRecord extends PersistenceRecord {
  id: string;
  version: string;
  states: string[];
  transitions: Record<string, Array<WorkflowTransitionDefinition | string>>;
  approval_gates?: string[];
  sla_rules?: JsonObject;
...
export interface ProcessProgressionAtomicPersistence extends PersistencePort {
  progressProcess?(input: {
    ...
    processPatch: JsonObject;
    tasks: ProcessTask[];
    event: JsonObject;
  }): Promise<{ process: ProcessInstance; tasks: ProcessTask[]; event?: PersistenceRecord; idempotent?: boolean }>;
}

export interface ProcessProgressionResult {
  process: ProcessInstance;
  nextTasks: ProcessTask[];
  transitionEvent: PersistenceRecord;
  idempotent: boolean;
}
...
export class ProcessProgressionRuntime {
  ...
  constructor(
    persistence: ProcessProgressionAtomicPersistence = new MemoryPersistenceAdapter(),
    events = new EventStore(persistence),
    queue = new TaskQueueRuntime({ persistence, events }),
  ) {
  ...
  async progress(processId: string, completedTaskId: string, actorId: string): Promise<ProcessProgressionResult> {
    ...
    const workflowId = this.workflowId(process);
    const workflowVersion = this.workflowVersion(process);
    const workflow = await this.requireWorkflow(workflowId, workflowVersion);
    ...
    const nextStatus = isTerminal ? "completed" : waitingApproval ? "waiting_approval" : "running";
    ...
    const processPatch: JsonObject = {
      ...
      status: nextStatus,
      state: { ...process.state, last_transition: { from: process.current_stage, to: nextStage, event: eventName, task_id: task.id } },
      completed_at: isTerminal ? new Date().toISOString() : null,
      version: String(Number(expectedVersion) + 1),
    };
    const idempotencyKey = `process-progress:${process.id}:${task.id}`;
    const event: JsonObject = {
      id: idempotencyKey,
      type: "PROCESS_TRANSITION_RESOLVED",
      ...
      outcome: nextStatus,
      provenance: { workflow_id: workflow.id, workflow_version: workflow.version, task_id: task.id },
      payload: { from: process.current_stage, to: nextStage, event: eventName, next_task_ids: nextTasks.map((candidate) => candidate.id) },
    };

    if (this.persistence.progressProcess) {
      const atomic = await this.persistence.progressProcess({ processId: process.id, expectedVersion, processPatch, tasks: nextTasks, event });
      return { process: atomic.process, nextTasks: atomic.tasks, transitionEvent: atomic.event ?? event, idempotent: atomic.idempotent ?? false };
    }
    ...
    return this.persistence.transaction(async () => {
      ...
      if (existingEvents.length) {
        const current = await this.requireProcess(process.id);
        const existingTasks = await this.persistence.query(TASK_COLLECTION, (record) => nextTasks.some((candidate) => candidate.id === record.id));
        return { process: current, nextTasks: existingTasks as unknown as ProcessTask[], transitionEvent: existingEvents[0], idempotent: true };
      }
  ...
  private buildNextTask(process: ProcessInstance, completedTask: ProcessTask, transition: WorkflowTransitionDefinition, eventName: string, index: number): ProcessTask {
    ...
    return {
      id: `${process.id}:${completedTask.id}:${eventName}:${index}`,
      version: "1",
      ...
      due_at: transition.due_at ?? null,
      payload: { ...(completedTask.payload ?? {}), ...(transition.payload ?? {}), source_task_id: completedTask.id, transition_event: eventName },
      requirements: { ...(transition.requirements ?? {}), ...(approvalRequired ? { approvalRequired: true } : {}) },
      outcome: {},
      evidence: [],
      created_at: now,
      updated_at: now,
  ...
  private resolveTransitions(workflow: WorkflowRecord, from: string, eventName: string): WorkflowTransitionDefinition[] {
    const candidates = workflow.transitions?.[from] ?? [];
    const matches = candidates.filter((candidate) => typeof candidate !== "string" && candidate.event === eventName) as WorkflowTransitionDefinition[];
    if (!matches.length) {
      if (candidates.some((candidate) => candidate === eventName)) throw new Error(`Transition target unresolved for ${from}:${eventName}`);
      throw new Error(`No executable transition for ${from}:${eventName}`);
    }
    return matches;
  }
  ...
  private async requireProcess(id: string): Promise<ProcessInstance> {
    const record = await this.persistence.read(PROCESS_COLLECTION, id);
    if (!record) throw new Error(`Process instance not found: ${id}`);
    return record as unknown as ProcessInstance;
  }
  ...
  private workflowId(process: ProcessInstance): string {
    const id = process.workflow_id ?? process.state?.["workflow_id"];
    if (typeof id !== "string" || !id) throw new Error("Process instance has no workflow_id");
    return id;
  }

  private workflowVersion(process: ProcessInstance): string {
    const version = process.workflow_version ?? process.state?.["workflow_version"];
    if (typeof version !== "string" || !version) throw new Error("Process instance has no workflow_version");
    return version;
  }
}
