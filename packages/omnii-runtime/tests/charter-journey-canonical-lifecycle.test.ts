...
test("Charter journey runtime follows the canonical lifecycle through Event Engine", async () => {
  ...
  for (const next of ["discovery", "eligibility", "availability", "reservation", "assignment", "preparation", "pickup", "active", "handoff", "completion", "evidence", "reconciliation", "closed"] as const) execution = await runtime.transition(execution, next);
...
test("Charter rejects an invalid journey transition", async () => {
  ...
  await assert.rejects(runtime.transition(runtime.create(journey.id), "active"), /Invalid journey transition/);
});
