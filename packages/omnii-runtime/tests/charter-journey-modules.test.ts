...
test("Charter preserves multimodal handoff semantics", async () => {
  ...
  let state = execution.create(journey.id); state = { ...state, state: "active", currentLeg: firstLeg.id };
...
test("Charter records evidence references without replacing the evidence source", async () => {
  ...
  assert.equal(attached.sourceRef, "telemetry:event-1"); assert.equal((await evidence.list(journey.id)).length, 1);
});
