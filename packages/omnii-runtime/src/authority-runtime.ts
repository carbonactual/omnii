...
export class AuthorityRuntime {
  ...
  private resourceAllowed(authority: AuthorityRecord, resourceId: string): boolean { const resources = Array.isArray(authority.constraints?.["resourceIds"]) ? authority.constraints["resourceIds"] as unknown[] : []; return resources.length === 0 || resources.includes(WILDCARD) || resources.includes(resourceId); }
  private contextAllowed(authority: AuthorityRecord, context: JsonObject): boolean { const required = authority.context ?? {}; return Object.entries(required).every(([key, value]) => context[key] === value); }
  private async require(id: string): Promise<AuthorityRecord> { const authority = await this.inspect(id); if (!authority) throw new Error(`Authority not found: ${id}`); return authority; }
}
