...
export class MissionAdaptationRuntime {
  ...
  adapt(
    ...
    for (const failedMemberId of failureIds) {
      ...
      if (assessmentBefore.readiness === "ready" || assessmentBefore.readiness === "incomplete") {
      ...
      const candidates = candidatePool
        ...
        .filter((candidate) => !candidate.dependsOn?.some((dependencyId) => {
          const dependency = working.find((member) => member.id === dependencyId) ?? candidatePool.find((item) => item.id === dependencyId);
          return !dependency || dependency.status === "unavailable" || dependency.status === "suspended";
        }))
        .sort((left, right) => {
      ...
      if (!replacement) {
        ...
        const finalAssessment = this.intelligence.assess(mission, working);
        return {
          status: "halted",
          members: working,
