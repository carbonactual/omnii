...
begin
  ...
  select * into v_process
   ...
   for update;
  if not found then raise exception 'Process instance not found: %', p_process_id; end if;
  if coalesce(v_process.version, '1') <> p_expected_version then raise exception 'Persistence version conflict: process_instances/%', p_process_id; end if;
  ...
  for v_task in select * from jsonb_array_elements(coalesce(p_tasks, '[]'::jsonb)) loop
    insert into public.omnii_process_tasks (
      ...
      v_task->>'id', v_task->>'process_id', v_task->>'stage', v_task->>'task_type', v_task->>'assignee_id', v_task->>'status',
      nullif(v_task->>'due_at', '')::timestamptz, coalesce(v_task->'payload', '{}'::jsonb), coalesce(v_task->'requirements', '{}'::jsonb),
      coalesce(v_task->'outcome', '{}'::jsonb), coalesce(v_task->'evidence', '[]'::jsonb),
      coalesce((v_task->>'created_at')::timestamptz, now()), coalesce((v_task->>'updated_at')::timestamptz, now()),
      coalesce(v_task->>'version', '1'), v_task->>'id'
  ...
  insert into public.omnii_events (
    ...
    coalesce(v_event_id, gen_random_uuid()::text), '1', 'active', coalesce(p_event->'authority', '{}'::jsonb), coalesce(p_event->'provenance', '{}'::jsonb),
    coalesce(p_event->'payload', '{}'::jsonb) || jsonb_build_object('type', p_event->>'type', 'actor', p_event->>'actor', 'outcome', p_event->>'outcome'),
    nullif(p_event->>'correlation_id', ''), coalesce((p_event->>'occurred_at')::timestamptz, now()), now(), v_event_idempotency, nullif(p_event->>'operating_context_id', '')
  ) returning * into v_existing_event;

  select * into v_process from public.omnii_process_instances where id = p_process_id;
  select coalesce(array_agg(id order by id), array[]::text[]) into v_task_ids from public.omnii_process_tasks where id in (select value->>'id' from jsonb_array_elements(coalesce(p_tasks, '[]'::jsonb)));
  return jsonb_build_object(
    'process', to_jsonb(v_process),
    'tasks', coalesce((select jsonb_agg(to_jsonb(t)) from public.omnii_process_tasks t where t.id = any(v_task_ids)), '[]'::jsonb),
    'event', to_jsonb(v_existing_event),
