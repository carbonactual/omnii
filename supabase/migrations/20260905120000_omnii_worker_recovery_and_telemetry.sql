...
create index if not exists omnii_worker_runs_started_idx on public.omnii_worker_runs (started_at desc, run_kind, status);
...
begin
  ...
  for v_task in
    select t.* from public.omnii_process_tasks t
     where t.status = 'in_progress' and t.lease_expires_at is not null and t.lease_expires_at <= now()
     order by t.lease_expires_at asc, t.created_at asc for update skip locked limit v_limit
  loop
    begin
      v_max_attempts := coalesce(v_task.max_attempts, 0);
      if v_max_attempts > 0 and coalesce(v_task.attempt_count, 0) >= v_max_attempts then
        perform public.omnii_escalate_process_task(v_task.id, v_task.version, v_worker_id,
          'Worker lease expired and retry limit was exhausted.', v_task.assignee_id);
        ...
        update public.omnii_process_tasks
           set status = 'ready', claimed_by = null, claimed_at = null, lease_expires_at = null,
               ...
               version = (version::integer + 1)::text, updated_at = now()
    ...
    exception when others then
      v_failed := v_failed + 1;
      update public.omnii_process_tasks
         set status = 'blocked', error_code = 'LEASE_RECOVERY_EXCEPTION', error_message = left(sqlerrm, 1000),
  ...
  update public.omnii_worker_runs
     set status = case when v_failed > 0 then 'failed' else 'succeeded' end,
         finished_at = now(), recovered = v_recovered, escalated = v_escalated, failed = v_failed,
         result = jsonb_build_object('recovered',v_recovered,'escalated',v_escalated,'failed',v_failed)
   where id = v_run_id;

  return jsonb_build_object('status', case when v_failed > 0 then 'failed' else 'ok' end,
    'worker_id', v_worker_id, 'recovered', v_recovered, 'escalated', v_escalated,
    'failed', v_failed, 'run_id', v_run_id, 'ran_at', now());
end;
$recover$;
  ...
  begin
    ...
    update public.omnii_worker_runs
       set status = case when v_status = 'failed' then 'failed' else 'succeeded' end,
           finished_at = now(), processed = coalesce((v_result->>'processed')::integer, 0),
           completed = coalesce((v_result->>'completed')::integer, 0), blocked = coalesce((v_result->>'blocked')::integer, 0),
           failed = coalesce((v_result->>'failed')::integer, 0), result = coalesce(v_result, '{}'::jsonb)
     where id = v_run_id;
    return coalesce(v_result, '{}'::jsonb) || jsonb_build_object('run_id', v_run_id);
  exception when others then
    update public.omnii_worker_runs set status='failed', finished_at=now(), failed=1, error=left(sqlerrm,1000) where id=v_run_id;
...
begin
  ...
  if exists (select 1 from cron.job where jobname='omnii-lease-recovery') then perform cron.unschedule('omnii-lease-recovery'); end if;
