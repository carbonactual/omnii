...
export interface ProcessInstance extends PersistenceRecord { process_type: string; subject_id?: string | null; form_submission_id?: string | null; status: string; current_stage: string; workflow_id?: string | null; workflow_version?: string | null; context: JsonObject; state: JsonObject; authority: JsonObject; evidence: JsonObject; started_at?: string | null; completed_at?: string | null; updated_at: string; }
...
export class ProcessProgressionRuntime {
  ...
  async progress(processId: string, completedTaskId: string, actorId: string): Promise<ProcessProgressionResult> {
    ...
    const processPatch: JsonObject = {
      ...
      state: {
        ...
        last_transition: { from: process.current_stage, to: nextStage, event: eventName, task_id: task.id },
      },
    ...
    const event: PersistenceRecord = {
      id: idempotencyKey,
      version: "1",
      lifecycle: "active",
      authority: {},
      provenance: { workflow_id: workflow.id, workflow_version: workflow.version, task_id: task.id },
      payload: { type: "PROCESS_TRANSITION_RESOLVED", actor: actorId, from: process.current_stage, to: nextStage, event: eventName, next_task_ids: nextTasks.map((candidate) => candidate.id) },
    ...
    return this.persistence.transaction(async () => {
      ...
      const createdEvent = await this.persistence.create("events", event);
  ...
  private buildNextTask(process: ProcessInstance, completedTask: ProcessTask, transition: WorkflowTransitionDefinition, eventName: string, index: number): ProcessTask {
    ...
    return {
      ...
      payload: { ...(completedTask.payload ?? {}), ...(transition.payload ?? {}), source_task_id: completedTask.id, transition_event: transition.event },
